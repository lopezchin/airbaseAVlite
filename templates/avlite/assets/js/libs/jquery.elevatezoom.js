...
(function( $, window, document, undefined ) {
	var ElevateZoom = {
			init: function( options, elem ) {
				...
				var timeVal = 2000;
				var zoomChildCount = $("#gal1 > li").size();
				var zoomItemStart =1;
				var timeFunc;
				


				//Create the image newImg2.src from the gallery 
				...
				/*timeFunc=setInterval(slideTimer, timeVal);

				function slideTimer(){
					

					if(zoomItemStart>zoomChildCount){
						zoomItemStart=1;
					}
					var zoomItemStart2=zoomItemStart++;
					
					var imgsrc = $('#gal1 li:nth-child('+ (zoomItemStart2) +') .zoom_trigger').data("zoom-image");

					//Set a class on the currently active gallery image
					if(self.options.galleryActiveClass){
						$('#'+self.options.gallery + ' a').removeClass(self.options.galleryActiveClass);
						$('#gal1 li:nth-child('+ (zoomItemStart2) +') .zoom_trigger').addClass(self.options.galleryActiveClass);
					}

					$('.slides #zoom-img').attr('src',imgsrc)
					$('.zoomLens img').attr('src',imgsrc)
					$('.zoomWindowContainer div').css('background-image','url(http://localhost/av-lite/'+ imgsrc +')')
			...
			swaptheimage: function(smallimage, largeimage){
				...
				newImg.onload = function() {
					self.largeWidth = newImg.width;
					self.largeHeight = newImg.height;
					self.zoomImage = smallimage
					self.swapAction(smallimage, largeimage);
					return;              
				}          
				newImg.src = smallimage; // this must be done AFTER setting onload
         ...
         if(self.options.zoomType == "lens") {
				...
				self.$elem.attr("src",smallimage);  
				self.$elem.attr("data-zoom-image",smallimage);
				$(".zoomLens img").attr("src",smallimage);    
			...
			doneCallback: function(){
				...
				if(self.options.tint) {
					self.zoomTintImage.attr("src",smallimage);
